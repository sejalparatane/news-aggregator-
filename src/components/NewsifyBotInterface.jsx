...
import { Send, Bot, User, FileText, Globe, CheckCircle, AlertCircle, Loader2 } from "lucide-react";
...
export default function ChatInterface() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { response, updated_history } = await sendMessageToAPI(input.trim(), history);
      ...
      setHistory(updated_history); // Update history with API response
    } catch (error) {
      console.error("Error:", error);
      const errorMessage = {
        id: (Date.now() + 1).toString(),
        content: { response_type: "text", text_response: "Sorry, there was an error processing your request. Please try again.", fake_news_report: null },
        role: "assistant",
        timestamp: Date.now(),
      };
          ...
          <Card className="border-theme-purple/20 shadow-md">
            ...
            <CardContent className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <Badge className={`text-white bg-theme-purple`} variant={credibilityScore > 70 ? "success" : credibilityScore > 40 ? "warning" : "destructive"}>
                  {credibilityScore}%
                </Badge>
              </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {isTyping && (
          <div className="flex items-center gap-2 max-w-[80%]">
            ...
            <div className="flex items-center gap-2 rounded-2xl bg-muted/50 px-4 py-2.5 text-sm text-theme-purple">
